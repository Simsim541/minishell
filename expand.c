...
/*   Updated: 2023/02/21 16:42:57 by mberri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	len_var(char *str, char **env, int *j)
{
	...
	if (!str || str[0] == '\0')
		return (0);
	while (str[len_var] && str[len_var] != ' '
		&& (ft_isalnum(str[len_var]) || str[len_var] == '_'))
		len_var++;
	*j += len_var;
	while (env && env[i])
	{
...
char	*expand_env(char *str, char **env)
{
	...
	cmdline = malloc(sizeof(char) * lenght + 3 * sizeof(int));
	if (cmdline == NULL)
		return (NULL);
	ft_memset(cmdline, 0, lenght);
	cmdline = fill_cmdline(str, env, cmdline, getpid());
	return (cmdline);
}
